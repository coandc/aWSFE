...
def compute_accuracy_fusion(the_args, b2_model, iteration, start_iter, fusion_vars, tg_model, tg_feature_model, class_means, evalloader, scale=None, print_info=True, device=None):
    if device is None:
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    tg_model.eval()
    tg_feature_model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(evalloader):
            ...
            if iteration == start_iter:
                ...
                outputs, outputs_feature = process_fusion_feature(the_args, fusion_vars, tg_model, b2_model, inputs)
            # outputs = tg_model(inputs)
            outputs = F.softmax(outputs, dim=1)
            if scale is not None:
                assert(scale.shape[0] == 1)
