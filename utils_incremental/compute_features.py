...
def compute_features_fusion(the_args, fusion_vars, tg_model, free_model, start_iteration, iteration, tg_feature_model, evalloader, num_samples, num_features, device=None):
    if device is None:
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    tg_feature_model.eval()
    tg_model.eval()
    ...
    with torch.no_grad():
        for inputs, targets in evalloader:
            ...
            if start_iteration == iteration:
                ...
                the_feature = process_fusion_feature(the_args, fusion_vars, tg_model, free_model, inputs, feature_mode=True)
