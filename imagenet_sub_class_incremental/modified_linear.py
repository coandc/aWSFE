...
class SplitLinear(Module):
    #consists of two fc layers and concatenate their outputs
    def __init__(self, in_features, out_features1, out_features2, sigma=False):
        super(SplitLinear, self).__init__()
        self.in_features = in_features
        self.out_features = out_features1 + out_features2
        self.fc1 = torch.nn.Linear(in_features, out_features1, True)
        self.fc2 = torch.nn.Linear(in_features, out_features2, True)
        if sigma:
            self.sigma = Parameter(torch.Tensor(1))
            self.sigma.data.fill_(1)
        else:
